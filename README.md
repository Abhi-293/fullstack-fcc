# FreeCodeCamp Full-Stack Web Development Practice Repository

This repository contains my practice projects and assignments from the **FreeCodeCamp Full-Stack Web Development** course. The course teaches building hireable skills in the JavaScript ecosystem, covering HTML, CSS, JavaScript, React, Node.js, Next.js, TypeScript, databases, and testing.

## Course Resources

- **Interactive Course on Scrimba**: [Full-Stack Web Development Path](https://scrimba.com/fullstack-path-c0...)
- **Challenge Code on GitHub**: [Scrimba Learn Full-Stack Repository](https://github.com/scrimba/learn-full...)
- **Scrimba on YouTube**: [Scrimba Channel](https://www.youtube.com/c/scrimba)

## Skills Covered

- HTML & CSS
- JavaScript (Basics, Advanced, Asynchronous Programming)
- React & Next.js
- Node.js & APIs
- TypeScript
- Databases (SQL)
- Git & GitHub
- Command Line Basics
- Testing and More

## Course Contents

The course is structured with lessons, challenges, and projects. Below is an outline based on the course timeline:

- **00:00:00** - Introduction
- **00:01:27** - HTML and CSS
- **01:04:19** - Intro to CSS
- **02:21:01** - Building a Business Card
- **03:32:02** - Space Exploration
- **04:17:21** - Birthday Website
- **05:22:25** - Solo Project: Hometown Homepage *(Recently completed and included in this repo)*
- **05:30:38** - Build a Counter App
- **06:46:17** - JavaScript Challenges Part 1
- **07:07:22** - Git and GitHub Basics
- **07:19:59** - Build a Blackjack Game
- **10:15:19** - JavaScript Challenges Part 2
- **10:41:09** - Build a Chrome Extension
- **13:34:25** - JavaScript Challenges Part 3
- **14:00:48** - Command Line Basics
- **14:46:27** - Essential Git and GitHub Skills
- **15:12:55** - Advanced Foundations
- **16:27:18** - Methods and Loops
- **17:41:43** - Functions, Expressions, and Parameters
- **18:26:57** - Asynchronous JavaScript and APIs
- **19:42:29** - AI Engineering Fundamentals
- **21:19:42** - Build a Node API
- **22:42:51** - Build a Full-Stack Node App
- **24:52:41** - Introduction to Databases
- **25:13:25** - Writing SQL Queries
- **26:30:04** - Static Pages
- **28:48:33** - Data Driven
- **31:03:00** - React State
- **36:17:02** - Side Effects
- **38:05:49** - Capstone Project
- **40:12:26** - TypeScript Fundamentals
- **42:15:21** - TypeScript in React
- **43:06:47** - Build a Next.js App
- **45:08:21** - Rendering Strategies and More

## Repository Structure

This repo includes practice implementations of the course projects. Currently, it features:

- `hometown-hompage/`: Contains the Hometown Homepage project (HTML, CSS, and related assets like `frontend.html`, `frontend.css`, `index.html`, and `frontend.jpg`).

As I progress through the course, more projects and challenges will be added here.

## How to Use

1. Clone or download this repository.
2. Navigate to specific project folders (e.g., `hometown-hompage/`) to view or run the code.
3. For interactive learning, follow along with the Scrimba course and refer to the original challenge code on GitHub.
4. Open HTML files in a browser to see static projects in action.

## Notes

- This is a personal practice repository for learning and experimentation.
- All projects are based on the FreeCodeCamp/Scrimba curriculum.
- Feel free to explore, but for the full experience, enroll in the course on Scrimba.

Happy coding! 🚀